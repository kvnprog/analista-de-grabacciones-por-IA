...
def upgrade():
    # 1. Creamos la función que hará el segundo insert
    op.execute("""
        CREATE OR REPLACE FUNCTION funcion_replicar_insert()
        RETURNS TRIGGER AS $$
        -- En Postgres, las variables se declaran ANTES del BEGIN
        DECLARE 
            v_username VARCHAR;
            v_client VARCHAR;
            v_role VARCHAR;
            ...
            v_campaign_id INT;
        BEGIN
            -- 1. Obtenemos los datos del usuario interno.
            ...
            -- 2. Obtenemos el nombre del cliente
            SELECT name INTO v_client 
            FROM public.internal_campaign 
            WHERE id = v_campaign_id;

            -- 3. Insertamos en la tabla de destino
    ...
    # 2. Creamos el trigger que se dispara DESPUÉS del insert
    op.execute("""
...
def downgrade():
    ...
    op.execute("DROP TRIGGER IF EXISTS tr_despues_de_insertar_usuario ON public.data_users;")
    op.execute("DROP FUNCTION IF EXISTS funcion_replicar_insert();")
